...
// 模板组装：用中文说明每个成分，不使用符号包裹，只保留非空部分
function assemble(act, line, mind) {
  const parts = [];
  if (line.trim()) parts.push(`台词（我说出口的话，NPC 能听到）：${line.trim()}`);
  if (act.trim()) parts.push(`动作（我做出的行为，NPC 能看到）：${act.trim()}`);
  if (mind.trim()) parts.push(`心理（我的内心想法，NPC 无法感知，不要在正文中引用或回应它）：${mind.trim()}`);
  if (!parts.length) return "";
  return "本轮我的输入包含以下成分，请严格区分，不要混为一谈：\n" + parts.join("\n");
}
...
function TriComposer(props) {
  ...
  const header = h("div", {
    ...
    h("span", { style: { opacity: 0.6 } }, "分框填写，自动用中文说明每个成分"),
  ...
  if (mode === "detail") {
    // 纵向（详细）：三个多行文本框纵排，块级布局，全部完整显示
    return h("div", { style: wrapStyle },
      header,
      h("div", { style: { padding: "0 8px 8px" } },
        FIELDS.map((f) =>
          h("div", { key: f.key, style: { display: "block", marginBottom: "8px" } },
            h("div", { style: { fontSize: "12px", opacity: 0.7, marginBottom: "3px" } }, f.label + " · " + f.hint),
            h("textarea", {
              style: {
                ...
                resize: "vertical", minHeight: "52px"
              },
              rows: 2,
              value: vals[f.key],
              placeholder: f.hint,
